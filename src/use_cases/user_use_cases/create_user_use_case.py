...
class CreateUserCustomerUseCase:
    def __init__(self, user_repository, user_entity:UserEntity):
        ...
        self.__user_entity.is_active = True #TODO: email confirmation sending

    async def process(self):
        existing_user_entity = await self.__user_repository.get_user_by_username_or_email(
                self.__user_entity.username,
                self.__user_entity.email,
            )
        ...
        if self.__user_email_as_username_bool:
            ...
            created_user_repo = self.__user_repository(self.__user_entity)
            created_user_entity = await created_user_repo.create_user()
